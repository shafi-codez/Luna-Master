 ...
 * @author Hector Veiga
 * @author Gerardo Travesedo testing
 *
 */
public class readQueue {
    ...
    private static int port = Constants.SOCKET_PORT ;
    ...
    static {
        ...
        try {
            File propFile = new File(Constants.AWS_PROP);
     ...
     *
     * @param body the rowID number in String format
     * @param receiptHandle the SQS rereceiptHandle of the SQS message which has
     * the above row ID inside
    ...
    public static void main(String[] args) throws Exception {
        prop.load(new FileInputStream(Constants.FACTOR_PROP));
 
        GetQueueUrlRequest qrequest = new GetQueueUrlRequest(prop.getProperty("QUEUE_NAME"));
        String url = getSqs().getQueueUrl(qrequest).getQueueUrl();

        new MasterSocket(getPort(), url).start();
        System.out.println("LunaCore: Establishing Communication Socket Launched");
        ...
        while (true) {
            try {
                ...
                while (true) {
                    System.out.println("LunaCore: Iterating: looking for SQS messages");
                    sqsResponse = getSqs().receiveMessage(rMessage);
                    System.out.println("LunaCore: =>"+sqsResponse);
                ...
                System.out.println("LunaCore: The " + rowID + " associated instance type is "+ typeOfInstance);
                ...
                for (int image = 0; image < images.size(); image++) {
                    ...
                    // wanted AMI
                    if (images.get(image).getDescription().contains(prop.getProperty("IMAGE_NAME"))) {
                ...
                request.setKeyName(prop.getProperty("KEY_NAME"));
                ArrayList<String> securityGroupIds = new ArrayList<String>();
                securityGroupIds.add(prop.getProperty("SEC_GROUP"));
                ...
                p.setAvailabilityZone(prop.getProperty("AVAIL_ZONE"));
    ...
    /**
     *
     * @param s the String[] of the content of the SQS message
    ...
    private static String whatTypeOfInstance(String[] s) {
        if (s.length == 2 && s[1].equals("ts")) {
            return Constants.MED_INSTANCE;
        } else { // s.length == 1
            return Constants.MICRO_INSTANCE;
        }
    }
     ...
     *
     * @param s The String Collection of the lines of the Script
     * @param delimiter the delimiter we want in String format
