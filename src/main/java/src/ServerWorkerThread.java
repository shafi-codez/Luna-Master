...
public class ServerWorkerThread implements Runnable {
	...
	public ServerWorkerThread(Socket socket, String url) {
            try{
		...
		getServerSocket().setSoTimeout(Integer.parseInt(prop.getProperty("SOCKET_TIME_OUT"))*60*1000);
		this.url = url;
                prop.load(new FileInputStream(Constants.FACTOR_PROP));
                
            }catch(Exception e){
                System.out.printf("Caught Exception in ServerWorkerThread :"+e.getMessage());
                e.getStackTrace();
            }
	}
	...
	public void run() {
		...
		try {
			...
			System.out.println("ServerWokerThread " + rowID+ ": Got the RowID sussccesfully");

			// listen the receiptHandle
			setReceipHandle(input.readUTF());
			System.out.println("ServerWokerThread " + rowID + ": receipHandle="+ getReceipHandle());

			// Giving 1 hour to the worker to send the status before this thread
			// will be closed
			getServerSocket().setSoTimeout(Integer.parseInt(prop.getProperty("WORKER_TIME_OUT"))*60*1000);

			// changing visibility to 11 hours
			ChangeMessageVisibilityRequest changeVisibility = new ChangeMessageVisibilityRequest(
                                url, this.receiptHandle, Integer.parseInt(prop.getProperty("SOCKET_TIME_OUT")) * 60 * 60);
			...
			DeleteMessageRequest delRequest = new DeleteMessageRequest(url,this.receiptHandle);
